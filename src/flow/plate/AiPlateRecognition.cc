...
bool WarpPlateQuad(const uint8_t* src, int srcWidth, int srcHeight,
    ...
    for (int y = 0; y < kPlateInputHeight; ++y) {
        ...
        for (int x = 0; x < kPlateInputWidth; ++x) {
            ...
            const double srcX =
                (projection.a * u + projection.b * v + projection.c) / denominator;
            const double srcY =
                (projection.d * u + projection.e * v + projection.f) / denominator;
...
bool AiPlateRecognition::RecognizeTarget(const VideoFramePtr& frame, AiDetectRstEl& target) {
    ...
    if (landmarks.size() == 4) {
        ...
        if (packed && transform.EnsureHostData(packed) && packed->GetData()) {
            const std::array<util::Point, 4> quad = {
            ...
            if (WarpPlateQuad(packed->GetData(), static_cast<int>(packed->GetWidth()),
                ...
                resized = std::make_shared<media::VideoFrame>(
                    kPlateInputWidth, kPlateInputHeight, packed->GetPixelFormat(),
    ...
    const size_t pixels = static_cast<size_t>(kPlateInputWidth) * kPlateInputHeight;
    ...
    std::string error;
    if (recognizer_->RunRecognizerTensor(input, {1, 3, kPlateInputHeight, kPlateInputWidth}, ocr,
                                        ocr_shape, color, color_shape, error) != util::ErrorEnum::Success ||
